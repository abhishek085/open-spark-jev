...
| R21 | efficiency: spark-s1 vs the same-size untrained model writing JSON / thinking | `scripts/run_slm_baseline.sh` | 60-call set, idle GPU | 12.6x (4B) and 13.2x (1.7B) faster than JSON, 0 tokens written vs 16; 4B accuracy 0.850 vs 0.833; 1.7B 0.783 vs 0.433 (14 malformed) | `runs/slm_baseline_*.json`, BENCHMARKS.md B33 |
